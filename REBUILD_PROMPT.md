...
### 3.5 HTTP API
Reproduce v1's surface (rename paths freely, keep capabilities): book CRUD + rename + soft delete + can-delete, upload (multipart), reprocess, resume, dismiss-error, page image, cover get/put (file or page), chapter audio (Range + ETag) and timeline, sentences list/edit/insert/delete/regenerate/approve + segment audio, mismatches, generate/stop/reassemble, per-voice and per-chapter regenerate/continue, voices add/remove, sample, summary detect/re-read, page text put, re-ocr, line-split/line-typos + SLM model list, TTS models/voices/servers, lexicon get/put, health. Async writes ack immediately and stream progress over WS; interactive tasks return their result on the response. Consistent error envelope `{error}`; 409 for busy/locked conflicts.
